...
const url = 'https://www.superheroapi.com/api.php/1431955867325889'; //api
const searchBox = document.getElementById('search'); //search box
const searchResultsContainer = document.getElementById('search-results-container');//search result container

handleClick();
function handleClick(){
  searchBox.addEventListener('keyup', handleSearchBar);
}
...
async function handleSearchBar(e){
  ...
  // click enter in the search bar
  if(e.keyCode === 13 && name.length > 0){
    handleEnter(name);
  }
  // check of the the length is 0
  if(name.length == 0){
    await clearResults();
  }
  else{
    ...
    if(data && data.response === 'success'){
      ...
      let favs = getFavs();
      // createElement and addEventListener will be called
